...
When a View Component is added to the stage within a Context's contextView, it is by default mediated automatically based on the mapping supplied to the MediatorMap when it was registered. In a basic mediator, the viewComponent property is injected with the component that is being mediated. In most cases, we want access to a strongly typed object to receive the benefits provided by using strongly typed objects.
...
public class GalleryLabelMediator extends Mediator implements IMediator
{
	...
	*/
	override public function onRegister():void
	{
		...
		//adding and event listener to the view component being mediated
		addEventListener( myCustomComponent, MyCustomEvent.DID_SOME_STUFF, handleDidSomeStuff)
	}
