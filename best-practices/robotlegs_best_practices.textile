...
# *"MVC+S Reference Implementation":#mvcs*
## "Context":#context
...
The framework supplies a default implementation based on the "Model-View-Controller":http://en.wikipedia.org/wiki/Model–view–controller meta-design pattern. This implementation provides a strong suggestion as to application structure and design. While it does make your application slightly less portable, it still aims to be as minimally invasive as possible in your concrete classes. By extending the "MVC+S":#mvcs implementation classes, you are supplied with numerous methods and properties for the sake of convenience.

You are never obligated to use the standard "MVC+S":#mvcs implementation with Robotlegs. You can use any part of it, none of it, or freely roll your own implementation to suit your needs. It is included to provide a proper reference implementation and a jump start to using Robotlegs.
...
For the purposes of tis document, we are going to examine Property injection, and how this is used within Robotlegs. There are two options for injecting properties into your class. You can use unnamed, or named injection:
...
h4(#mappingwithinjector). Injection Mapping with the Injection Class
...
mapValue(MyClass, myClassInstance);
--
...
<pre>//someplace in your application where mapping/configuration occurs
mapSingletonOg(IMyClass, MyClass); //MyClass implements IMyClass
--
//in the first class to receive injections
[Inject]
...
<pre>//someplace in your application where mapping/configuration occurs
mapView(MyAwesomeWidget, MyAwesomeWidgetMediator); 
--
...
<pre>//someplace in your application where mapping/configuration occurs
mapEvent(MyCoolCommand, MyAppDataEvent.DATA_WAS_RECEIVED, MyAppDataEvent); 
--
...
<pre>addEventListenerTo(eventDispatcher, SomeEvent.IT_IS_IMPORTANT, handleFrameworkEvent)</pre>
...
A Mediator is responsible for listening to events dispatched by the view component being mediated. This can be a single component, such as a TextField or Button, or a complex hierarchy of nested components. When a view component event has been added to a mediator it will be handled by the method designated to handle the event. As with framework events, the _addEventListenerTo_ convenience method is the preferred syntax for adding event listeners to a mediator:

<pre>addEventListenerTo(myMediatedViewComponent, SomeEvent.USER_DID_SOMETHING, handleUserDidSomethingEvent)</pre>
