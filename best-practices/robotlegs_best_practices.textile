...
A Mediator is responsible for listening to events dispatched by the view component being mediated. This can be a single component, such as a TextField or Button, or a complex hierarchy of nested components. When a view component event has been added to a mediator it will be handled by the class method designated to handle the event. As with framework events, the _addEventListenerTo_ convenience method is the prefferred syntax for adding event listeners to a mediator:
